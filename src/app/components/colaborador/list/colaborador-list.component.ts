...
import { ModalVisualizarColaboradorComponent } from 'src/app/shared/modais/modal-visualizar-colaborador/modal-visualizar-colaborador.component';
...
export class ColaboradorListComponent extends Pagination<ColaboradorFilter> implements OnInit {
  ...
  public async  onClickOpenModalVisualizar(id: number): Promise<void> {
    this.messageService.clearAllMessages();
    const initialState = {
      colaborador: (await this.service.findById(id).toPromise()).result
    };
    this.modalService.show(ModalVisualizarColaboradorComponent, { initialState, backdrop: 'static' });
  }
